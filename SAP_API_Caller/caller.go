...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-identity-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) AsyncGetIdentityGetLoggedInUserInfo(userID string, accepter []string) {
	wg := &sync.WaitGroup{}
	wg.Add(len(accepter))
...
func (c *SAPAPICaller) IdentityGetLoggedInUserInfo(userID string) {
	identityGetLoggedInUserInfoData, err := c.callIdentitySrvAPIRequirementIdentityGetLoggedInUserInfo("IdentityGetLoggedInUserInfoData", userID)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": identityGetLoggedInUserInfoData, "function": "IdentityGetLoggedInUserInfoData"})
	if err != nil {
		c.log.Error(err)
		return
	}
